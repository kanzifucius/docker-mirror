...
func (m *mirror) getRemoteTags() ([]RepositoryTag, error) {
	...
	var allTags []RepositoryTag
	tagPage, err := getTagsPage(url)
	if err != nil {
		return nil, err
	}

	allTags = tagPage.Results

	for tagPage.Next != nil {
		tagPage, err := getTagsPage(url)
		if err != nil {
			return nil, err
		}

		allTags = append(allTags, tagPage.Results...)
	}
...
func getTagsPage(url string) (TagsResponse, error) {
	var tagPage TagsResponse
	...
	if err := json.NewDecoder(r.Body).Decode(&tagPage); err != nil {
		return tagPage, err
	}
